...
### TPMs vs. read counts vs. ... ?

RATs will give you results regardless of what type of abundances you give it. It is *your responisbility* to decide on the
appropriate abundance type *and* adjust the parameters and thresholds to match the properties of the abundances.

Our recommendation is to use transcript counts rather than read counts. Read counts are influenced by the length of the transcripts,
which affects the isoform abundance profile and thus the DTU. TPMs are normalised transcript counts and in principle should work, but 
be warned that the DTU calls are likely to be conservative as the TPM numbers are generallly small. If you have an idea on how to 
extrapolate the number of transcript copies in a sample from the TPMs and/or read counts, those would be ideal.
...
RATs offers a method to convert [Salmon](https://combine-lab.github.io/salmon/) output to [Kallisto](https://pachterlab.github.io/kallisto/) format using [wasabi](https://github.com/COMBINE-lab/wasabi) `prepare_fish_for_sleuth()`, and then import the bootstrapped read counts and convert them to TPMs.
...
2. `annot` - The annotation table you used for quantification and that you will use for calling DTU.
