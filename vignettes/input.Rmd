...
-> [Sleuth](http://pachterlab.github.io/sleuth/) objects contain their input data, so RATs can extract the bootstrapped abundance estimates directly from there, for simplicity.
...
The following thresholds are available in RATs:
...
3. `dprop_thresh` - Efect size threshold. Transcripts whose proportion changes by less than the threshold between conditions, are considered non-DTU, regardless of their statistical significance.
...
In this process, one quantification iteration will be randomly selected from each sample and DTU will be called on it. This will be repeated `qbootnum`. Because the
...
Low reproducibility indicates that the quantification tool found it hard to distinguish these transcripts. This can be caused by high similarity of isoforms, genes with high
number of isoforms, and/or poor read coverage in the regions differentiating the isoforms from one another. In these cases skepticism is required over the quantifications
and any potential differential expression regarding these transcripts.

Warnings will be generated if `qbootnum` is below 100 or higher than the number of iterations in your input data, but RATs will proceed with the analysis. The warnings
...
be easily underestimated or overestimated, so we recommend not using the conservative mode in these cases. Instead use this reproducibility as additional information.
...
Although we expect Sleuth field names to be consistent, with the exception of covariate names, RATs allows overriding the
expected names.
