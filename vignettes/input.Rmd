...
4. Salmon/Kallisto quantifications.
...
-> [Salmon](https://combine-lab.github.io/salmon/) and [Kallisto](https://pachterlab.github.io/kallisto/) quantifications are currently accessed using the 
[wasabi](https://github.com/COMBINE-lab/wasabi) converter and the Sleuth data importer. This has the benefit that it covers both formats and that data gets 
normalised, but at the cost of generating a temporary Sleuth object, so this approach will require considerable memory.
...
The method we recommend is to to convert [Salmon](https://combine-lab.github.io/salmon/) output to [Kallisto](https://pachterlab.github.io/kallisto/) format using [wasabi](https://github.com/COMBINE-lab/wasabi) `prepare_fish_for_sleuth()`, load them into R with [Sleuth](http://pachterlab.github.io/sleuth/) `sleuth_prep()`. From there either run RATs with the Sleuth object, or use the memory-saving extraction process described in the previous section.

The reason for this round-about way is that (a) it covers all the formats and (b) importing via Sleuth creates estimated transcript counts, which
is the type of abundance that should be used with RATs. Raw read counts are not suitable, as they are not normalised for sequence and length biases and sequencing depth. TPMs are also not suitable because the values are too small to work with a counts-based distribution.

We do recognise that such procedure is several extra steps and that it can be scripted. So RATs provides a wrapper that does those things for you. The catch is that you will have to repeat the process manually if you want to call DTE with Sleuth.
...
2. `annot` - The annotation table you will use for calling DTU. This must be the same throughout RATs.

The return value of `fish4rodents` is a list with two items: `$boot_data_A` and `$boot_data_B`.
...
![RATs design](./fig/RATs_decisions.jpg)
...
2. `COUNTS_COL` - The name of the field holding the abundances in the bootstrap tables of Sleuth. (Default "est-counts")
...
The `COUNTS_COL` and `BS_TARGET_COL` parameters are also available for `denest_sleuth_boots()` and `fish4rodents()`.
