...
`rats` aims to identify situations where genes with mulitple transcript isoforms show differences in which of 
...
To inder which genes show change in relative abundance of their transcripts, the *G test* is used. This method is 
...
two tests are carried out independently from each other, meaning that a positive result in one is not a pre-requisite 
for running the other and, indeed, they two tests lead to conflicting verdicts for some genes.

The **input format** recognized by `rats` is the output of [sleuth](http://pachterlab.github.io/sleuth/). See the 
...
*Your data may have more/fewer/different variables* in the `sample_to_covariates` table. `rats` uses this table as
guide to group samples together for comparison.
...
and subject to different restrictions. They are complementary information to one another.
...
7. `tests` Which of the two tests were carried out.
8. `bootstrap` Which of the two tests were bootstrapped.
...
results at the gene level. For your convenience, the transcript-level DTU calls are also summarized here.
...
18. `boot_stdev` The the standard deviation of the mean P-value across the bootstraps.
...
21. `boot_na` The fraction of bootstrap iterations in which the transcripts was not eligible for DTU calling.
...
* `ALLB` has two transcript, both detected, yet is still not eligible as both transcripts are not eligible. Inspection of
...
**Note:** The values shown in the bootstrap columns will differ in each run of the ame command, especially for few iterations,
...
Several mentions to this table were made during analysis of the `Genes` table. Here are some possible scenarios:

* `CC_a` and `CC_b` both show counts (`sumA` and `sumB` above the threshold (`Parameters$counts_thresh`) so they are eligible. 
They are positive for DTU at both the gene and transcript level. The frequency of positive call in the bootstraps, however,
is not quite good. Additionally, the change in proportion is only marginally above the already quite permissive threshold.
...
Examples 1 and 2 have mentioned the possibility that some transcripts/genes may be missing altogether from either the
annotation or the sleuth object. This is likely to be cause by use of different annotation versions during different
...
conditions are low-count improves the confidence in the predictions of the remaining transcripts, as well as reduces 
computation time.
...
(faster, less sensitive, more explicit report). They are complementary, and **we recomend using them
...
A variation of the above is to plot the mean counts instead of the proportions. Either can also be displayed as
a frequency polygon instead.

```{r}
# The error bars for proportions represent the standard error of the estimated proportions.
plot_gene(mydtu, "MIX6", vals="counts", style="lines")
```
...
You can save any of the plots as a `ggplot2` object and use [ggplot2](http://ggplot2.org) manipulations on it, such as changing the axes scales.
