...
`rats` uses the *proportions test* to find which transcripts of a gene change expression relative 
...
For your convenience, we provide a couple of functions to give you a quick summary of your results, without your
having to inspect and subset the full results tables. However, we do recommend you become familiar with the
results structure and content.
...
different restrictions. They are complementary to one another.
...
7. `elig` Eligible for testing: A gene needs at least 2 eligible transcripts for the G test 
to be applicable and for DTU to even be possible.
...
results at the transcript level. For your convenience, the gene-leveel DTU calls are also included here.
...
more than one eligible expression transcripts (by DTU definition), and finally the gene needs to have detectable 
...
10. `propA` and `propB` The proportional abundance of the transcript in each of the two conditions.
11. `Dprop` The difference in proportional abundance of the transcript between the two conditions.
...
13. `pval` The raw P-value for the transcript.
14. `pval_corr` The above P-value adjusted for multiple testing (number of transcripts).
15. `sig` Statistically significant: `pval < Parameters$p_thresh`.
...
* `ALLA` has one transcript and it is detected but one transcript is not enough to be eligible.
* `ALLB` has two transcript, both detected, yet is still not eligible as both transcripts are not eligible.
...
* `ALLA` and `ALLB` are similar in that they are detected in one condition only, with none of their transcripts detected in
the other. Proportions cannot be defined for that condition (division by 0), so they are not eligible.
...
**You should avoid mixing annotations** throughout your analysis pipeline, as there is no guarantee (at least for 
some annotations) that the transcript identification codes will remain consistent between annotation versions.
