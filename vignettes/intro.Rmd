...
abstract: >
  ...
  particularly with **RNA-Seq** type of data in mind. This vignette gives installation instructions and a very brief overview of usage.
vignette: >
...
quantification method, although it was originally meant to be used in workflows that use [Kallisto](http://pachterlab.github.io/kallisto/), 
[Sailfish](http://github.com/kingsfordgroup/sailfish) or [Salmon](https://github.com/COMBINE-lab/salmon) as isoform quantification tools, 
followed by DTE by [Sleuth](http://pachterlab.github.io/sleuth).
...
Available as R source package form from the project's [releases section](https://github.com/bartongroup/Rats/releases) on Github.
Download the latest release and then install it using:

`install.packages("<path/to/dowloaded/package>", repos = NULL, type="source")`

Release snapshots are labelled with even-ending versions numbers (x.x.0/2/4/6/8).
...
# 6. Plot isoform changes for the top DTU gene returned by the previous step.
plot_gene(mydtu, myids[["dtu-genes"]][1])
```

The input and output structures and the command options are discussed in more detail in the tutorial vignette.
...
are quite permissive. This means you are likely to get an inflated number of positive calls.

For explanations of the available thresholds, refer to the tutorial vignette.
