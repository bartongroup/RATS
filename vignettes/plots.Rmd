...
With the use of the third-party `ggbio` package, one can plot gene models from GRanges/GRangesList objects. This can be useful in interpreting transcript expression and DTU, but getting an annotation sliced up in a suitable way can be a little tedious.

RATs offers a helper function to create an appropriate collection of `GRanges`/`GRangesList` objects from a GTF file. The collection is structured as a list (indexed by gene ID), where each element is a `GRangesList` object containing a `GRanges` per transcript of that gene.
...
For more options and more refined plots, refer to the [ggbio](https://bioconductor.org/packages/release/bioc/html/ggbio.html) documentation.
