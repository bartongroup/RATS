...
For package introduction, installation and quick-start, please refer to the introduction vignette.
...
From a [Sleuth](http://pachterlab.github.io/sleuth/) object `rats` extracts the bootstrapped abundance estimates. To find
out more about Sleuth-based workflows refer to the follwoing links:
...
A rudimentary function is provided to create this table, given a GTF file. Currently, it has been tested only on a limited number of GTF files,
so be sure to give the output a quick look to satisfy yourself that the GTF was parsed correctly. **Note:** GFF3 is *not* supported, as the 
specification is too relaxed regarding identifiers.
...
To prevent cluttering this tutorial with verbose output, we will use this option in all the examples, but we recommend you leave
this feature switched on, as turning it off will also suppress some warnings.

If you leave `verbose = TRUE` when trying out the examples below using the emulated data, you will get **warnings**
about the number of bootstraps. The warnings are meant to guard against poor use of `rats`. In this specific case, 
the warning is triggered because the emulated dataset used in the examples immitates only the structure of 
...
21. `boot_p_max` - The maximum (least significant) P-value that occurred in the bootstraps.
...
are essentially the same as the input.
...
1. `condA` - The averaged counts across bootstraps, for eash transcript and replicate in the first condition.
2. `condB` - The averaged counts across bootstraps, for eash transcript and replicate in the other condition.
...
styles that have been kept for consistency or demonstration purposes. 
...
The volcano plot and the isoform abundance change plots mentioned above are also available through a `shiny` app. This is a great way to
explore your data and identify interesting transcripts.
...
the isoform abundance change plots for the point nearest to the click.
...
annotation or the counts data. This is likely to be caused by use of different annotation versions during different
