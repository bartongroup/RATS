...
# Relative Abundance of Transcripts (RATs v0.2.2).
...
starting with stricter thresholds.

For explanations of the thresholds, refer to the relevant section(s) further down the tutorial.
...
To prevent cluttering this tutorial, we will use this option in all the examples, but we recommend you leave
this feature switched on, as turning it off will also suppress some warnings.

If you leave `verbose = TRUE` when trying out the examples below with the emulated data, you will get **warnings**
about the number of bootstraps. This is because the dataset used in the examples only emulates the structure of 
real data, not the actual volume of it, and as such contains very few iterations indeed.
...
proportions are plotted for each transcript. The data is shown as both a box plot and as points representing 
...
Several styles of this plot are available, to allow you to pick the one that best suits your visualisation 
preferences. The aim is to include information about transcript level DTU and replicate of origin, alongside the
