...
1. The point shapes represent the replicates, to allow finding the related points across isoforms. The **maximum allowed number of 
replicates per condition** is 25, which should be ample for typical use. 
2. The line colour encodes the transcript-level DTU calls, to highlight the relevant isoforms. Remember that it is possible for a
a positive gene-level DTU prediction even if every individual isoform is predicted negative, as different tests are used for each level.
3. The fill colour allows the easy identification of an isoform across all facets of the plot.
