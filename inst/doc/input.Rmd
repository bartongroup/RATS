...
2. `half_cooked` - If FALSE, call wasabi to prepare Salmon format for Sleuth. Wasabi automatically skips a conversion when an `abundance.h5` file
is present in that sample directory, so leaving it FALSE will probably work for all purposes.
...
The `TARGET_COL` and `PARENT_COL` parameters are also available for `denest_sleuth_boots()`.
`TARGET_COL` is also available as a parameter for `fish4rodents()`.
